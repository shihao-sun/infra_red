...
int Begin_run = 0;
int Stop = 0;
int The_Gray_Value[PICTURE][HEIGHT][WIDTH];
int Picture_GV[HEIGHT][WIDTH];
float skewness[HEIGHT][WIDTH];

int getskewness(int num[100][192][256],float skewness[192][256]);    /*getskewness函数功能：求偏度*/
void createAlphaMat(Mat &mat,float skewness[192][256]);
void produce_picture(float skewness[192][256]);    //生成偏度图片
void rendersigner(int flag);
    ...
    while(uvcState)
    {
        for(n_buffers = 0; n_buffers < 4; n_buffers++)
        {
            ...
            if(rightData)
            {
                ...
                if(write_flag >= 90 && write_flag < 390)    //从第二帧的照片开始把灰度值写入三维数组，写100张
                {
                    for(int i = 0;i < bgrImg.rows;i++)
                    {
                        for(int j = 0;j < bgrImg.cols;j++)
                        {
                             The_Gray_Value[point_flag][i][j] = bgrImg.at<uchar>(i , j); //把灰度值赋值给三维数组
                        }
                    }
                    point_flag++;
                 }
                ...
                char str[20];
                sprintf(str,"./image/%d.png",point_flag + 1);

             //   produce_picture(skewness);
                ...
                imwrite(str,bgrImg,compression_params);
                if(point_flag == 300)
                {
                     getskewness(The_Gray_Value,skewness);   //计算偏度
                     produce_picture(skewness);     //生成图片        
                     point_flag++;
                }
    ...
    int ret=0;
     /* Py_Initialize();
        if(!Py_IsInitialized())
          ...
          int argc = 1;
          wchar_t* argv[1];
          char* str = "./Run_Infrared_Thermal.py";
          argv[0] = (wchar_t*)str;
          PySys_SetArgv(argc, argv);

          if(PyRun_SimpleString("execfile('./Run_Infrared_Thermal.py')") == NULL)
          {
              std::cout <<"2" <<std::endl;
              return -1;
          }*/
